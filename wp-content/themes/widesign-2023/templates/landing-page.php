<?php

/*
 * Template Name: Landing Template
 * Template Post Type: post, page, press-release, people
 */

get_header(); ?>

<!-- wysiwyg main content -->
<div class="bg-gray-100">
    <!-- HERO -->
    <section class="flex flex-wrap md:flex-nowrap overflow-hidden">
        <div class="w-full md:w-2/4">
            <div class="pb-1/3 lg:pb-4/5 xl:pb-2/3 2xl:pb-3/5 relative">
                <img class="absolute top-0 left-0 h-full md:h-100 lg:h-full w-full object-cover" src="/resources/img/pages/services/widesign-small-business.jpg" alt="WideSign small business startup">
            </div>
        </div>

        <div class="relative py-8 md:py-0 w-full md:w-2/3 flex justify-center items-center bg-gray-900">
            <div class="hex-container">

                <div class="row">
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                </div>
                <div class="row">
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                </div>
                <div class="row">
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                </div>
                <div class="row">
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                </div>
                <div class="row">
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                </div>
                <div class="row">
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                </div>
                <div class="row">
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                </div>
                <div class="row">
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                </div>

                <div class="row">
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                </div>
                <div class="row">
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                    <div class="hexagon"></div>
                </div>

            </div>
            <div class="relative p-3 md:p-10 text-4xl lg:text-5xl lg:p-6">
                <h2 class="pb-1 text-mellow-200 uppercase font-semibold">
                    Start your small
                </h2>
                <h2 class="pb-1 text-mellow-400 uppercase font-semibold">
                    Business website today!
                </h2>
                <h2 class="pb-1 text-mellow-600 uppercase font-semibold">
                    Just £249
                </h2>
                <div class="grid justify-items-start mt-2">
                    <a class="relative inline-flex pr-8 primary-btn  hover:bg-teal-500 bg-gray-800 group" href="#tell-me-more">
                        Whats included
                        <svg xmlns="http://www.w3.org/2000/svg" class="absolute ml-1 top-3 right-1 inline-block transform group-hover:rotate-360 transition-rotate duration-500 w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M9 5l7 7-7 7" />
                        </svg>
                    </a>
                </div>
            </div>
        </div>
    </section>

    <!-- WEBDESIGN -->
    <section id="tell-me-more" class="py-10 md:py-20 px-4 md:px-10 bg-teal-600 text-center">
        <h1 class="text-gray-200 text-4xl uppercase font-bold">
            Business startup package
        </h1>

        <!-- INLCUDES -->
        <section id="includes-web" class="pb-2 md:pb-8 px-4 md:px-10">
            <h1 class="text-xl text-center pt-2 pb-10">Includes:</h1>
            <!-- <p class="text-sm text-center pb-10">
                All features <a href="#" class="modal-trigger relative text-plum-200 primary-link underline text-sm" data-id="modal-features">in website packages</a>
            </p> -->

            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 grid-flow-row gap-5 max-w-5xl mx-auto">

                <div class="flex flex-col text-gray-200 rounded-xl border-2 border-teal-700 p-3 gap-2">
                    <div class="flex justify-between relative">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="absolute -top-6 -left-6 text-mellow-400 flex-none w-10 h-10">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M5.25 14.25h13.5m-13.5 0a3 3 0 01-3-3m3 3a3 3 0 100 6h13.5a3 3 0 100-6m-16.5-3a3 3 0 013-3h13.5a3 3 0 013 3m-19.5 0a4.5 4.5 0 01.9-2.7L5.737 5.1a3.375 3.375 0 012.7-1.35h7.126c1.062 0 2.062.5 2.7 1.35l2.587 3.45a4.5 4.5 0 01.9 2.7m0 0a3 3 0 01-3 3m0 3h.008v.008h-.008v-.008zm0-6h.008v.008h-.008v-.008zm-3 6h.008v.008h-.008v-.008zm0-6h.008v.008h-.008v-.008z" />
                        </svg>
                        <header class="text-center text-lg font-semibold px-3 w-full text-white">Hosting</header>
                    </div>

                    <div class="flex flex-col pl-3">
                        <div class="card-content pb-2">
                            <p class="leading-5 text-sm">We deploy and host your website to our secure UK servers once design and build are complete. All our packages include the first year of hosting and domain name FREE!</p>
                        </div>
                    </div>

                </div>
                <div class="flex flex-col text-gray-200 rounded-xl border-2 border-teal-700 p-3 gap-2">
                    <div class="flex justify-between relative">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="absolute -top-6 -left-6 text-mellow-400 flex-none w-10 h-10">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M10.5 1.5H8.25A2.25 2.25 0 006 3.75v16.5a2.25 2.25 0 002.25 2.25h7.5A2.25 2.25 0 0018 20.25V3.75a2.25 2.25 0 00-2.25-2.25H13.5m-3 0V3h3V1.5m-3 0h3m-3 18.75h3" />
                        </svg>
                        <header class="text-center text-lg font-semibold px-3 w-full text-white">Domain &amp; Email</header>
                    </div>

                    <div class="flex flex-col pl-3">
                        <div class="card-content pb-2">
                            <p class="leading-5 text-sm">We set up your domain name with associated professional email address and can help you with setting it up on your phone or laptop.</p>
                        </div>
                    </div>

                </div>

                <div class="flex flex-col text-gray-200 rounded-xl border-2 border-teal-700 p-3 gap-2">
                    <div class="flex justify-between relative">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="absolute -top-6 -left-6 text-mellow-400 flex-none w-10 h-10">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M9 17.25v1.007a3 3 0 01-.879 2.122L7.5 21h9l-.621-.621A3 3 0 0115 18.257V17.25m6-12V15a2.25 2.25 0 01-2.25 2.25H5.25A2.25 2.25 0 013 15V5.25m18 0A2.25 2.25 0 0018.75 3H5.25A2.25 2.25 0 003 5.25m18 0V12a2.25 2.25 0 01-2.25 2.25H5.25A2.25 2.25 0 013 12V5.25" />
                        </svg>
                        <header class="text-center text-lg font-semibold px-3 w-full text-white">Professional design</header>
                    </div>


                    <div class="flex flex-col pl-3">

                        <div class="card-content pb-2">
                            <p class="leading-5 text-sm">All website designs incorporate your existing logo and brand (If you have these already). Creating a clean interface with structured goals and navigation.</p>
                        </div>
                    </div>

                </div>


                <!-- <div class="flex flex-col text-gray-200 rounded-xl border-2 border-teal-700 p-3 gap-2">
                    <div class="flex justify-between relative">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="absolute -top-6 -left-6 text-mellow-400 flex-none w-10 h-10">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M10.5 1.5H8.25A2.25 2.25 0 006 3.75v16.5a2.25 2.25 0 002.25 2.25h7.5A2.25 2.25 0 0018 20.25V3.75a2.25 2.25 0 00-2.25-2.25H13.5m-3 0V3h3V1.5m-3 0h3m-3 18.75h3" />
                        </svg>
                        <header class="text-center text-lg font-semibold px-3 w-full text-white">Responsive design</header>
                    </div>
                    <div class="flex flex-col pl-3">
                        <div class="card-content pb-2">
                            <p class="leading-5 text-sm">Using a mobile first approach we develop all sites to be as fast a possible on mobile. Adjusting across multiple screen sizes and browsers, whatever the device.</p>
                        </div>
                    </div>

                </div> -->

                <div class="flex flex-col text-gray-200 rounded-xl border-2 border-teal-700 p-3 gap-2">
                    <div class="flex justify-between relative">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="absolute -top-6 -left-6 text-mellow-400 flex-none w-10 h-10">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M16.5 10.5V6.75a4.5 4.5 0 10-9 0v3.75m-.75 11.25h10.5a2.25 2.25 0 002.25-2.25v-6.75a2.25 2.25 0 00-2.25-2.25H6.75a2.25 2.25 0 00-2.25 2.25v6.75a2.25 2.25 0 002.25 2.25z" />
                        </svg>
                        <header class="text-center text-lg font-semibold px-3 w-full text-white">SSL certificates</header>
                    </div>
                    <div class="flex flex-col pl-3">
                        <div class="card-content pb-2">
                            <p class="leading-5 text-sm">Security is of utmost importance. We include a free SSL security certificate with every site that hosted on our secure servers using TLS network protocol to establish an encrypted connection. </p>
                        </div>
                    </div>

                </div>

                <div class="flex flex-col text-gray-200 rounded-xl border-2 border-teal-700 p-3 gap-2">
                    <div class="flex justify-between relative">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="absolute -top-6 -left-6 text-mellow-400 flex-none w-10 h-10">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 00-3.375-3.375h-1.5A1.125 1.125 0 0113.5 7.125v-1.5a3.375 3.375 0 00-3.375-3.375H8.25m5.231 13.481L15 17.25m-4.5-15H5.625c-.621 0-1.125.504-1.125 1.125v16.5c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 00-9-9zm3.75 11.625a2.625 2.625 0 11-5.25 0 2.625 2.625 0 015.25 0z" />
                        </svg>
                        <header class="text-center text-lg font-semibold px-3 w-full text-white">Search Engine Optimization</header>
                    </div>

                    <div class="flex flex-col pl-3">
                        <div class="card-content pb-2">
                            <p class="leading-5 text-sm">We use tried-and-tested SEO techniques to give your site the edge in today's saturated search engines. We'll help you increase your online presence in major search engines.</p>
                        </div>
                    </div>

                </div>




                <div class="flex flex-col text-gray-200 rounded-xl border-2 border-teal-700 p-3 gap-2">
                    <div class="flex justify-between relative">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="absolute -top-6 -left-6 text-mellow-400 flex-none w-10 h-10">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M11.42 15.17L17.25 21A2.652 2.652 0 0021 17.25l-5.877-5.877M11.42 15.17l2.496-3.03c.317-.384.74-.626 1.208-.766M11.42 15.17l-4.655 5.653a2.548 2.548 0 11-3.586-3.586l6.837-5.63m5.108-.233c.55-.164 1.163-.188 1.743-.14a4.5 4.5 0 004.486-6.336l-3.276 3.277a3.004 3.004 0 01-2.25-2.25l3.276-3.276a4.5 4.5 0 00-6.336 4.486c.091 1.076-.071 2.264-.904 2.95l-.102.085m-1.745 1.437L5.909 7.5H4.5L2.25 3.75l1.5-1.5L7.5 4.5v1.409l4.26 4.26m-1.745 1.437l1.745-1.437m6.615 8.206L15.75 15.75M4.867 19.125h.008v.008h-.008v-.008z" />
                        </svg>
                        <header class="text-center text-lg font-semibold px-3 w-full text-white">Backups &amp; Support</header>
                    </div>

                    <div class="flex flex-col pl-3">
                        <div class="card-content pb-2">
                            <p class="leading-5 text-sm">Websites should rarely have downtime, but just in case, we store monthly backups of your website allowing us to restore your website in an emergency. Support for all your services with us by email.</p>
                        </div>
                    </div>
                </div>

                <!-- <div class="flex flex-col text-gray-200 rounded-xl border-2 border-teal-700 p-3 gap-2">
                    <div class="flex justify-between relative">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="absolute -top-6 -left-6 text-mellow-400 flex-none w-10 h-10">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M16.712 4.33a9.027 9.027 0 011.652 1.306c.51.51.944 1.064 1.306 1.652M16.712 4.33l-3.448 4.138m3.448-4.138a9.014 9.014 0 00-9.424 0M19.67 7.288l-4.138 3.448m4.138-3.448a9.014 9.014 0 010 9.424m-4.138-5.976a3.736 3.736 0 00-.88-1.388 3.737 3.737 0 00-1.388-.88m2.268 2.268a3.765 3.765 0 010 2.528m-2.268-4.796a3.765 3.765 0 00-2.528 0m4.796 4.796c-.181.506-.475.982-.88 1.388a3.736 3.736 0 01-1.388.88m2.268-2.268l4.138 3.448m0 0a9.027 9.027 0 01-1.306 1.652c-.51.51-1.064.944-1.652 1.306m0 0l-3.448-4.138m3.448 4.138a9.014 9.014 0 01-9.424 0m5.976-4.138a3.765 3.765 0 01-2.528 0m0 0a3.736 3.736 0 01-1.388-.88 3.737 3.737 0 01-.88-1.388m2.268 2.268L7.288 19.67m0 0a9.024 9.024 0 01-1.652-1.306 9.027 9.027 0 01-1.306-1.652m0 0l4.138-3.448M4.33 16.712a9.014 9.014 0 010-9.424m4.138 5.976a3.765 3.765 0 010-2.528m0 0c.181-.506.475-.982.88-1.388a3.736 3.736 0 011.388-.88m-2.268 2.268L4.33 7.288m6.406 1.18L7.288 4.33m0 0a9.024 9.024 0 00-1.652 1.306A9.025 9.025 0 004.33 7.288" />
                        </svg>
                        <header class="text-center text-lg font-semibold px-3 w-full text-white">Ongoing support</header>
                    </div>

                    <div class="flex flex-col pl-3">
                        <div class="card-content pb-2">
                            <p class="leading-5 text-sm">You receive support for all your services with us by email or mobile 24/7, and we enjoy going the extra mile to resolve requests within 24hrs. We won't leave you in the lurch.</p>
                        </div>
                    </div>
                </div> -->

                <!-- <div class="flex flex-col text-gray-200 rounded-xl border-2 border-teal-700 p-3 gap-2">
                    <div class="flex justify-between relative">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="absolute -top-6 -left-6 text-mellow-400 flex-none w-10 h-10">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M9 12h3.75M9 15h3.75M9 18h3.75m3 .75H18a2.25 2.25 0 002.25-2.25V6.108c0-1.135-.845-2.098-1.976-2.192a48.424 48.424 0 00-1.123-.08m-5.801 0c-.065.21-.1.433-.1.664 0 .414.336.75.75.75h4.5a.75.75 0 00.75-.75 2.25 2.25 0 00-.1-.664m-5.8 0A2.251 2.251 0 0113.5 2.25H15c1.012 0 1.867.668 2.15 1.586m-5.8 0c-.376.023-.75.05-1.124.08C9.095 4.01 8.25 4.973 8.25 6.108V8.25m0 0H4.875c-.621 0-1.125.504-1.125 1.125v11.25c0 .621.504 1.125 1.125 1.125h9.75c.621 0 1.125-.504 1.125-1.125V9.375c0-.621-.504-1.125-1.125-1.125H8.25zM6.75 12h.008v.008H6.75V12zm0 3h.008v.008H6.75V15zm0 3h.008v.008H6.75V18z" />
                        </svg>
                        <header class="text-center text-lg font-semibold px-3 w-full text-white">Content management system</header>
                    </div>

                    <div class="flex flex-col pl-3">
                        <div class="card-content pb-2">
                            <p class="leading-5 text-sm">All sites come CMS ready so content management is simplified and easily scalable. We offer demos and tutorials on blogging &amp; making changes if you need some guidance. Alternatively, join our maintenance packages, and we will make changes for you.</p>
                        </div>
                    </div>
                </div> -->

                <!-- <div class="flex flex-col text-gray-200 rounded-xl border-2 border-teal-700 p-3 gap-2">
                    <div class="flex justify-between relative">

                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="absolute -top-6 -left-6 text-mellow-400 flex-none w-10 h-10">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M3 13.125C3 12.504 3.504 12 4.125 12h2.25c.621 0 1.125.504 1.125 1.125v6.75C7.5 20.496 6.996 21 6.375 21h-2.25A1.125 1.125 0 013 19.875v-6.75zM9.75 8.625c0-.621.504-1.125 1.125-1.125h2.25c.621 0 1.125.504 1.125 1.125v11.25c0 .621-.504 1.125-1.125 1.125h-2.25a1.125 1.125 0 01-1.125-1.125V8.625zM16.5 4.125c0-.621.504-1.125 1.125-1.125h2.25C20.496 3 21 3.504 21 4.125v15.75c0 .621-.504 1.125-1.125 1.125h-2.25a1.125 1.125 0 01-1.125-1.125V4.125z" />
                        </svg>
                        <header class="text-center text-lg font-semibold px-3 w-full text-white">Analytics &amp; tags</header>
                    </div>
                    <div class="flex flex-col pl-3">
                        <div class="card-content pb-2">
                            <p class="leading-5 text-sm">We can set up tools like 'Google Analytics' and 'Google Search Console' for you. Also, any other tracking or social media tags required can be implemented using Google Tag Manager for maintainability.</p>
                        </div>
                    </div>
                </div> -->

                <!-- <div class="text-gray-200 rounded-xl border-2 border-teal-700 p-2 border-2 border-gray-200 rounded-md">

                <div class="flex flex-col p-6">
                    <header class="text-right text-lg font-semibold pr-2 text-mellow-200">Full plan features</header>
                    <div class="card-content pb-2">
                        <p class="leading-5 text-sm">Extra features are avaiable to some plans. See full <a href="#" class="modal-trigger relative text-mellow-200 primary-link underline" data-id="modal-features">feature list</a> for more info.</p>
                    </div>
                </div>
            </div> -->
            </div>
        </section>

        <!-- <div class="max-w-xl mx-auto py-3">
            <p class="text-gray-200 rounded-xl border-2 border-teal-700 p-2">
              Set up of Hosting & Domain name
            </p>
            <p class="text-gray-200">
                We can help with integrating your existing branding/logo or help you recreate a fresh new concept for your needs. We work together to create a user-friendly journey with your business goals in mind, and plan a site navigation that works for you.
            </p>
        </div> -->


    </section>

    <!-- SEO -->
    <!-- <section class="flex flex-wrap md:flex-nowrap overflow-hidden">
        <div class="w-full md:w-3/5">
            <div class="pb-2/3 lg:pb-4/5 2xl:pb-3/5  relative"><img class="absolute top-0 right-0 h-full md:h-156 lg:h-full w-full object-cover" src="/resources/img/pages/services/widesign-seo.jpg" alt="Free website audit" /></div>
        </div>
        <div class="w-full md:w-2/5 flex justify-center items-center bg-plum-600">
            <div class="py-10 px-4 md:px-10">
                <h2 class="pb-2 text-3xl text-gray-200 uppercase font-semibold">SEO management</h2>
                <p class="pb-2 text-gray-200">We only use white hat techniques and proven SEO formulas to increase your visibility in popular search engines <span class="px-2 rounded bg-plum-700 inline-block">#google</span>.</p>
                <p class="pb-2 text-gray-200">Working short & long term with clients, we produce <span class="px-2 rounded bg-plum-700 inline-block">successful results</span> and maintain web vitals and ranking. Regular check ups and content refreshes, help to maintain gains made.</p>
                <p class="pb-5 text-gray-200 hidden lg:block">
                    You can view our option of SEO plans or get in touch if you're looking for more specific requirements.</p>


                <div class="flex space-x-4 mt-2">
                    <a class="js-slidein relative inline-flex pr-8 primary-btn hover:bg-plum-500 bg-plum-700 group" href="/services/seo-plans">View SEO plans<svg xmlns="http://www.w3.org/2000/svg" class="absolute ml-1 top-3 right-1 inline-block transform group-hover:rotate-360 transition-rotate duration-500 w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M9 5l7 7-7 7" />
                        </svg>
                    </a>
                    <a class="js-slidein-2 relative inline-flex pr-8 primary-btn hover:bg-plum-500 bg-plum-700 group" href="/services/free-website-audit">FREE audit<svg xmlns="http://www.w3.org/2000/svg" class="absolute ml-1 top-3 right-1 inline-block transform group-hover:rotate-360 transition-rotate duration-500 w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M9 5l7 7-7 7" />
                        </svg>
                    </a>
                </div>
            </div>
        </div>
    </section> -->

    <!-- UI/UX -->
    <!-- <section class="flex flex-wrap md:flex-nowrap overflow-hidden">
        <div class="w-full lg:w-2/5 flex md:justify-center items-center bg-teal-700">
            <div class="py-10 px-4 md:px-10">
                <h2 class="pb-2 text-3xl text-gray-200 uppercase font-semibold">UI/UX Design</h2>
                <h3 class="text-teal-400 text-xl text-bold">Usability is the key to success</h3>
                <div class="max-w-xl mx-auto py-3">
                    <p class="text-gray-200">Usability is a key consideration when building any application. Confusing the user can be a single point of failure for any website.
                    </p>
                    <p class="text-gray-200">Designing user interfaces that work for the user and the client is an art form.</p>
                    <p class="text-gray-200 md:hidden xl:block">We utilize Google Analytics and use tested methods to funnel users to the end goal & increase engagement, whilst maintaining a simple to use interface.</p>
                </div>

                <div class="flex justify-start mt-2 space-x-4">
                    <a class="js-slidein relative inline-flex pr-8 primary-btn hover:bg-teal-500 bg-teal-600 group" href="/our-work">Our work<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" stroke="none" stroke-width="2" class="absolute ml-1 top-3 right-1 inline-block transform group-hover:rotate-360 transition-rotate duration-500 w-6 h-6">
                            <path fill-rule="evenodd" d="M16.28 11.47a.75.75 0 010 1.06l-7.5 7.5a.75.75 0 01-1.06-1.06L14.69 12 7.72 5.03a.75.75 0 011.06-1.06l7.5 7.5z" clip-rule="evenodd" />
                        </svg></a>
                </div>
            </div>
        </div>
        <div class="w-full md:w-3/5">
            <div class="pb-2/3 lg:pb-4/5 2xl:pb-3/5  relative"><img decoding="async" class="filter transition-all duration-500 grayscale hover:grayscale-0 absolute top-0 right-0 h-full md:h-100 lg:h-full w-full object-cover" src="/resources/img/pages/services/widesign-ux-ui.jpg" alt="User friendly interfaces"></div>
        </div>
    </section> -->

    <section class="p-10 bg-gray-300 py-24">
        <div class="md:items-center flex flex-wrap md:flex-nowrap container mx-auto md:justify-center md:space-x-10">
            <div>
                <h2 class="text-3xl text-gray-700 uppercase font-semibold">
                    Ready to start?
                </h2>
                <p class="text-gray-700 text-base md:text-center sm:text-left">
                    Lets get you on the road to success.
                </p>
            </div>
            <div class="flex-none mt-3">
                <a class="js-slidein relative inline-flex pr-8 primary-btn-lg hover:bg-gray-600 bg-gray-700 group" href="/startup-form">Lets get in touch<svg xmlns="http://www.w3.org/2000/svg" class="absolute ml-1 top-3 right-1 inline-block transform group-hover:rotate-360 transition-rotate duration-500 w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M9 5l7 7-7 7" />
                    </svg>
                </a>
            </div>

        </div>
        <div class="border-t border-gray-500 mt-5 md:items-center flex flex-wrap md:flex-nowrap container mx-auto md:justify-center md:space-x-10">
            <div class="flex-none mt-6 text-gray-700 text-base">Alternatively, just give us a call on <br /><a class="text-gray-700 underline text-3xl" href='tel:<?php echo do_shortcode("[mobile_number]"); ?>'><?php echo do_shortcode("[mobile_number]"); ?></a></div>

    </section>

    <section id="" class="py-20 px-4 md:px-10 bg-teal-600 text-center">
        <div class="flex flex-wrap md:no-wrap justify-center mt-2 space-x-4 space-y-4 sm:space-y-0">


            <div class="md:w-auto">
                <a class="js-slidein relative inline-flex pr-8 primary-btn hover:bg-teal-500 bg-teal-700 group" href="/our-work" target="_blank">
                    Previous examples
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" stroke="none" stroke-width="2" class="absolute ml-1 top-3 right-1 inline-block transform group-hover:rotate-360 transition-rotate duration-500 w-6 h-6">
                        <path fill-rule="evenodd" d="M16.28 11.47a.75.75 0 010 1.06l-7.5 7.5a.75.75 0 01-1.06-1.06L14.69 12 7.72 5.03a.75.75 0 011.06-1.06l7.5 7.5z" clip-rule="evenodd" />
                    </svg>
                </a>
            </div>

            <div class="md:w-auto">
                <a class="js-slidein-2 relative inline-flex pr-8 primary-btn hover:bg-teal-500 bg-teal-700 group" href="/services/website-packages">
                    More advanced packages
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" stroke="none" stroke-width="2" class="absolute ml-1 top-3 right-1 inline-block transform group-hover:rotate-360 transition-rotate duration-500 w-6 h-6">
                        <path fill-rule="evenodd" d="M16.28 11.47a.75.75 0 010 1.06l-7.5 7.5a.75.75 0 01-1.06-1.06L14.69 12 7.72 5.03a.75.75 0 011.06-1.06l7.5 7.5z" clip-rule="evenodd" />
                    </svg>
                </a>
            </div>
        </div>
    </section>


</div>

<?php get_footer(); ?>